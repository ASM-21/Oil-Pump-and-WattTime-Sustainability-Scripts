...
class ActivePowerAnalyzer:
            ...
            if row['PartKindId'] != current_partkind:
                ...
                if current_partkind is not None and segment_start_idx < idx:
                    ...
                    program_boundaries.append({
                        ...
                        'total_power': segment_data['ActivePower'].sum(),
    ...
    def plot_sequential_programs(self, combined_timeline, program_boundaries, part_number, part_type):
        ...
        for i, boundary in enumerate(active_programs):
            ...
            if prog_name not in program_color_map:
                ...
                # Create legend entry
                uuid_short = str(boundary['partkind_uuid'])[:8]
                legend_label = f"{prog_name} (UUID: {uuid_short}...)"
        ...
        # Add compact labels with much more staggering to avoid overlap
        # Use 6 different height levels for better separation
        y_label_positions = [0.06, 0.12, 0.18, 0.24, 0.30, 0.36]  # 6 staggered heights
        label_height_idx = 0
        ...
        for boundary in program_boundaries:
            ...
            # Place label at bottom with staggered heights to avoid overlap
            mid_time = (boundary['start'] + boundary['end']) / 2
            y_max = ax.get_ylim()[1]
            y_pos = y_max * y_label_positions[label_height_idx % len(y_label_positions)]
            label_height_idx += 1
            
            # Compact label with better visibility
            label_text = f"{boundary['program_name']}\n{boundary['duration']:.0f}s"
        ...
        total_power = combined_timeline['ActivePower'].sum()
        ...
        ax.set_title(f'Sequential Active Power Timeline - {part_type.upper()} Part #{part_number}\n'
                     f'{len(active_boundaries)} Programs Detected | Total Duration: {total_duration:.1f}s | '
                     f'Total Power: {total_power:.2f} | Avg Power: {avg_power:.2f}',
                     fontsize=16, pad=20, fontweight='bold')
        ax.set_xlabel('Time (seconds)', fontsize=14, fontweight='bold')
        ax.set_ylabel('Active Power', fontsize=14, fontweight='bold')
    ...
    def print_program_summary(self, program_boundaries, part_number, part_type):
        """
        Print a summary table of all programs detected from UUIDs
        """
        print(f"\n{'='*100}")
        ...
        if active_programs:
            ...
            total_power = sum(p['total_power'] for p in active_programs)
            
            print(f"\nActive Programs: {len(active_programs)}")
            print(f"Total Duration (active): {total_duration:.1f} seconds ({total_duration/60:.1f} minutes)")
            print(f"Total Power (active): {total_power:.3f}")
            print(f"Average Power: {total_power/total_duration:.3f}")
            ...
            for boundary in active_programs:
                ...
                print(f"{boundary['program_name']:<20} {uuid_display:<40} {boundary['start']:<12.1f} "
                      ...
                      f"{boundary['avg_power']:<15.3f}")
            
            print(f"{'-'*120}")
            
            # Find program with highest power consumption
            max_power_program = max(active_programs, key=lambda x: x['avg_power'])
            print(f"\nHighest Avg Power: {max_power_program['program_name']} "
                  f"with {max_power_program['avg_power']:.3f} average power")
        ...
        if none_programs:
            print(f"\n{'='*100}")
            print(f"CHANGEOVER/IDLE PERIODS (NONE Programs):")
            print(f"{'='*100}")
            ...
            print(f"\n{'-'*100}")
            print(f"{'SEGMENT':<10} {'START (s)':<12} {'END (s)':<12} {'DURATION (s)':<15} {'AVG POWER':<15}")
            print(f"{'-'*100}")
            
            for i, boundary in enumerate(none_programs, 1):
                print(f"#{i:<9} {boundary['start']:<12.1f} {boundary['end']:<12.1f} "
                      f"{boundary['duration']:<15.1f} {boundary['avg_power']:<15.3f}")
...
def main():
    """
    Main function for analyzing active power with sequential program analysis
    """
    print("="*100)
    print("ENHANCED ACTIVE POWER ANALYZER - SEQUENTIAL PROGRAM ANALYSIS")
    print("="*100)
    ...
    if result:
        ...
        print(f"\nDetected {len(active_programs)} active programs and {len(none_programs)} changeover periods")
