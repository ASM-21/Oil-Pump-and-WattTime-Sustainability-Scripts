...
class ActivePowerAnalyzer:
    ...
    def extract_paired_data(self):
        """
        Extract processKindId (UUID), partKindId, and active power values at each timestamp
        ...
        self.log(f"Found {len(active_power_data)} active power entries")
        ...
        # Convert active power to numeric and handle negatives
        ...
        timeline_data['Operation'] = timeline_data['UUID'].apply(
            lambda x: self.uuid_to_operation.get(str(x).strip().upper(), 'NONE' if str(x).strip().upper() == 'NONE' else f'Unknown')
        )
        
        # Map PartKindId to program name
        timeline_data['Program'] = timeline_data['PartKindId'].apply(
            lambda x: self.partkind_to_program.get(str(x).strip().upper(), f'Unknown')
        )
    ...
    def analyze_part_programs(self, part_number, part_type='body', base_directory='.'):
        ...
        # Plot detailed operation view for Program 1
        self.plot_program_operations(combined_timeline, program_boundaries, part_number, part_type, program_number=1)
    ...
    def detect_operation_boundaries(self, program_data):
        """
        Detect individual operation boundaries within a program based on UUID changes
        """
        ...
        for idx, row in program_data.iterrows():
            if row['UUID'] != current_uuid:
                # End previous segment if it exists
                if current_uuid is not None and segment_start_idx < idx:
                    segment_data = program_data.iloc[segment_start_idx:idx]
                    
                    operation_name = segment_data['Operation'].iloc[0]
                    total_energy_kwh = segment_data['energy_kwh'].sum()
                    
                    operation_boundaries.append({
                        ...
                        'total_energy_kwh': total_energy_kwh,
                        'avg_power': segment_data['ActivePower'].mean(),
                        'max_power': segment_data['ActivePower'].max(),
                        'data_points': len(segment_data)
                    })
        ...
        # Handle the last segment
        if current_uuid is not None and segment_start_idx < len(program_data):
            segment_data = program_data.iloc[segment_start_idx:]
            operation_name = segment_data['Operation'].iloc[0]
            total_energy_kwh = segment_data['energy_kwh'].sum()
        ...
        return operation_boundaries
    
    def plot_program_operations(self, combined_timeline, program_boundaries, part_number, part_type, program_number=1):
        """
        Plot detailed operation-level view for a specific program
        Shows individual CAM operations color-coded by operation type
        """
        print(f"\nCreating detailed operation plot for Program {program_number}...")
        
        # Find the target program boundary - look for actual PROGRAM_X not NONE
        target_program = None
        for boundary in program_boundaries:
            # Skip NONE programs and look for matching program number in the name
            if boundary['program'] != 'NONE' and boundary['program'] == program_number:
                # Double check it's actually PROGRAM_X_Body/Lid pattern
                if f'PROGRAM_{program_number}' in boundary['program_name']:
                    target_program = boundary
                    break
        
        if target_program is None:
            print(f"ERROR: Program {program_number} not found in boundaries")
            print("Available programs:")
            for b in program_boundaries:
                if b['program'] != 'NONE':
                    print(f"  - {b['program_name']} (program={b['program']})")
            return
        
        # Filter timeline data for this program
        program_data = combined_timeline[
            (combined_timeline['Time_Seconds_Adjusted'] >= target_program['start']) &
            (combined_timeline['Time_Seconds_Adjusted'] <= target_program['end'])
        ].copy()
        
        if program_data.empty:
            print(f"ERROR: No data found for Program {program_number}")
            return
        
        # Reset index to avoid iloc issues
        program_data = program_data.reset_index(drop=True)
        
        # Detect operation boundaries
        operation_boundaries = self.detect_operation_boundaries(program_data)
        
        print(f"Found {len(operation_boundaries)} operations in Program {program_number}")
        
        # Create the plot
        fig, ax = plt.subplots(figsize=(24, 9))
        
        # Plot the power line
        power_line = ax.plot(program_data['Time_Seconds_Adjusted'], 
                            program_data['ActivePower'],
                            linewidth=2.5, color='#1f77b4', label='Active Power', zorder=5)
        
        # Define colors for different operation types - improved color palette
        operation_colors = {
            'CAVITY': '#DC143C',      # Crimson for cavity milling
            'SPOT': '#FF8C00',        # Dark orange for spotting
            'DRILL': '#1E90FF',       # Dodger blue for drilling
            'TAP': '#9370DB',         # Medium purple for tapping
            'FINISH': '#20B2AA',      # Light sea green for finishing
            'FACE': '#FFD700',        # Gold for facing
            'MILL': '#FF6347',        # Tomato for milling
            'PROFILE': '#FF1493',     # Deep pink for profiling
            'DEBUR': '#32CD32',       # Lime green for deburring
            'ENGRAV': '#4169E1',      # Royal blue for engraving
            'WALL': '#FF4500',        # Orange red for wall operations
            'PLANAR': '#8B008B',      # Dark magenta for planar
            'FLOOR': '#DAA520',       # Goldenrod for floor
            'NONE': '#808080'         # Gray for unknown
        }
        
        def get_operation_color(operation_name):
            """Determine color based on operation type"""
            operation_upper = operation_name.upper()
            for key, color in operation_colors.items():
                if key in operation_upper:
                    return color
        ...
        for op_boundary in operation_boundaries:
            op_name = op_boundary['operation']
            if op_name not in operation_color_map:
                color = get_operation_color(op_name)
                operation_color_map[op_name] = color
                
                # Create legend entry
                legend_label = f"{op_name} - {op_boundary['total_energy_kwh']:.4f} kWh"
                patch = plt.Rectangle((0, 0), 1, 1, fc=color, alpha=0.3, 
                                     edgecolor=color, linewidth=2)
                legend_patches.append((patch, legend_label))
        
        # Plot shaded backgrounds for each operation
        for op_boundary in operation_boundaries:
            color = operation_color_map[op_boundary['operation']]
            
            # Add shaded background
            ax.axvspan(op_boundary['start'], op_boundary['end'],
                      color=color, alpha=0.25, zorder=1)
            
            # Add vertical line at operation start
            ax.axvline(op_boundary['start'], color='darkred', linestyle='--', 
                      linewidth=1, alpha=0.5, zorder=3)
        
        # Add final vertical line
        if operation_boundaries:
            ax.axvline(operation_boundaries[-1]['end'], color='darkred', 
                      linestyle='--', linewidth=1, alpha=0.5, zorder=3)
        
        # Set title and labels
        total_energy = sum(op['total_energy_kwh'] for op in operation_boundaries)
        ax.set_title(f'Detailed Operation Timeline - Program {program_number} ({target_program["program_name"]})\n'
                    f'{part_type.upper()} Part #{part_number} | '
                    f'Duration: {target_program["duration"]:.1f}s | '
                    f'Total Energy: {total_energy:.4f} kWh | '
                    f'{len(operation_boundaries)} Operations',
                    fontsize=16, pad=20, fontweight='bold')
        ax.set_xlabel('Time (seconds)', fontsize=14, fontweight='bold')
        ax.set_ylabel('Active Power (W)', fontsize=14, fontweight='bold')
        ax.grid(True, alpha=0.3, linestyle=':', linewidth=1)
        
        # Add legend
        all_handles = [power_line[0]]
        all_labels = ['Active Power']
        ...
        ax.legend(all_handles, all_labels, 
                 ...
                 framealpha=0.95,
                 edgecolor='black',
                 title=f'Program {program_number} Operations',
                 title_fontsize=11)
        ...
        self.print_operation_summary(operation_boundaries, program_number, part_number, part_type)
    
    def print_operation_summary(self, operation_boundaries, program_number, part_number, part_type):
        """
        Print a summary table of all operations in the program
        """
        print(f"\n{'='*120}")
        print(f"OPERATION SUMMARY - Program {program_number} ({part_type.upper()} Part #{part_number})")
        print(f"{'='*120}")
        
        total_duration = sum(op['duration'] for op in operation_boundaries)
        total_energy = sum(op['total_energy_kwh'] for op in operation_boundaries)
        
        print(f"\nTotal Operations: {len(operation_boundaries)}")
        print(f"Total Duration: {total_duration:.1f} seconds ({total_duration/60:.1f} minutes)")
        print(f"Total Energy: {total_energy:.4f} kWh")
        
        print(f"\n{'-'*120}")
        print(f"{'OPERATION':<35} {'START (s)':<12} {'END (s)':<12} {'DURATION (s)':<15} "
              f"{'ENERGY (kWh)':<15} {'AVG POWER (W)':<15}")
        print(f"{'-'*120}")
        
        for op in operation_boundaries:
            print(f"{op['operation']:<35} {op['start']:<12.1f} {op['end']:<12.1f} "
                  f"{op['duration']:<15.1f} {op['total_energy_kwh']:<15.4f} "
                  f"{op['avg_power']:<15.2f}")
        
        print(f"{'-'*120}")
        
        # Find operation with highest energy
        max_energy_op = max(operation_boundaries, key=lambda x: x['total_energy_kwh'])
        print(f"\nHighest Energy Consumption: {max_energy_op['operation']} "
              f"with {max_energy_op['total_energy_kwh']:.4f} kWh")
        
        # Find longest operation
        longest_op = max(operation_boundaries, key=lambda x: x['duration'])
        print(f"Longest Duration: {longest_op['operation']} "
              f"with {longest_op['duration']:.1f}s")
        
        # Find highest power operation
        max_power_op = max(operation_boundaries, key=lambda x: x['avg_power'])
        print(f"Highest Avg Power: {max_power_op['operation']} "
              f"with {max_power_op['avg_power']:.2f} W")
    ...
    def plot_sequential_programs(self, combined_timeline, program_boundaries, part_number, part_type):
        ...
        # Define highly visible, distinct colors for programs
        colors = [
            '#E63946',  # Vivid red
        ...
        for boundary in program_boundaries:
            ...
            # Check if this is Unknown or NONE
            is_unknown = boundary['program'] == 'NONE' or 'Unknown' in prog_name
            ...
            if is_unknown:
                # Simpler label for unknown/changeover
        ...
        total_duration = combined_timeline['Time_Seconds_Adjusted'].max()
        ...
        ax.set_title(f'Sequential Active Power Timeline - {part_type.upper()} Part #{part_number}\n'
                     f'{num_actual_programs} Programs | Total Duration: {total_duration:.1f}s | '
    ...
    def print_program_summary(self, program_boundaries, part_number, part_type):
        ...
        none_programs = [p for p in program_boundaries if p['program'] == 'NONE']
        ...
        if none_programs:
            print(f"\n{'='*100}")
            print(f"CHANGEOVER/IDLE PERIODS (Combined NONE segments):")
            print(f"{'='*100}")
            
            total_none_duration = sum(p['duration'] for p in none_programs)
            total_none_energy_kwh = sum(p['total_energy_kwh'] for p in none_programs)
            avg_none_power = sum(p['avg_power'] * p['duration'] for p in none_programs) / total_none_duration if total_none_duration > 0 else 0
            
            print(f"Number of changeover periods: {len(none_programs)}")
            print(f"Total changeover time: {total_none_duration:.1f} seconds ({total_none_duration/60:.1f} minutes)")
            print(f"Total changeover energy (all NONE combined): {total_none_energy_kwh:.4f} kWh")
            print(f"Average power during changeovers: {avg_none_power:.2f} W")
            
            print(f"\n{'-'*100}")
            print(f"Individual Changeover Segments:")
            print(f"{'SEGMENT':<10} {'START (s)':<12} {'END (s)':<12} {'DURATION (s)':<15} {'ENERGY (kWh)':<15}")
            print(f"{'-'*100}")
            
            for i, boundary in enumerate(none_programs, 1):
                print(f"#{i:<9} {boundary['start']:<12.1f} {boundary['end']:<12.1f} "
                      f"{boundary['duration']:<15.1f} {boundary['total_energy_kwh']:<15.4f}")
            
            print(f"{'-'*100}")
            print(f"{'TOTAL':<10} {'':<12} {'':<12} {total_none_duration:<15.1f} {total_none_energy_kwh:<15.4f}")
            print(f"{'-'*100}")
...
def main():
    """
    Main function for analyzing active power and energy with sequential program analysis
    Calculates energy consumption (kWh) by integrating power over time
    """
    print("="*100)
    print("ENHANCED ACTIVE POWER & ENERGY ANALYZER - SEQUENTIAL PROGRAM ANALYSIS")
    print("="*100)
    print("This analyzer plots active power for all programs of a part sequentially")
    print("Calculates energy consumption (kWh) by integrating power measurements over time")
    print("Uses partKindId (UUID) to detect actual program boundaries and changeover times")
    print("Creates detailed operation-level analysis for individual programs")
    print("="*100)
    ...
    if result:
        ...
        none_programs = [b for b in program_boundaries if b['program'] == 'NONE']
        total_energy = sum(b['total_energy_kwh'] for b in active_programs)
        changeover_energy = sum(b['total_energy_kwh'] for b in none_programs)
        
        # Count actual programs (1-4 for body, 1-2 for lid)
        num_actual_programs = 4 if part_type.lower() == 'body' else 2
        
        print(f"\nAnalyzed {num_actual_programs} programs with {len(none_programs)} changeover periods")
        print(f"Total data points: {len(combined_timeline)}")
        print(f"Total program energy: {total_energy:.4f} kWh")
        print(f"Total changeover energy: {changeover_energy:.4f} kWh")
        print(f"Total combined energy: {total_energy + changeover_energy:.4f} kWh")
    else:
