...
first go at fixing the tables and images for better ness
...
def plot_all_regions_seasonal(df_hourly: pd.DataFrame, signal: str, figures_dir: Path):
    ...
    # Compute global ylim across all region/season combos
...
def main():
    ...
    # Compute global y-limits for consistent scaling across regions
    hourly_by_hour = df_hourly.groupby("hour")["value_mean"]
    hourly_stats_all = hourly_by_hour.agg(["mean", "std"])
    
    _ymin = hourly_by_hour.mean().min()
    _ymax = hourly_by_hour.mean().max()
    _margin = (_ymax - _ymin) * 0.05
    global_hourly_ylim = (_ymin - _margin, _ymax + _margin)
    
    # Wider bounds for plots with std shading
    global_hourly_ylim_wide = (
        (hourly_stats_all["mean"] - hourly_stats_all["std"]).min() * 0.95,
        (hourly_stats_all["mean"] + hourly_stats_all["std"]).max() * 1.05
    )
    
    # Global heatmap color range
    global_heatmap_range = (
        df_hourly.groupby(["hour", "month"])["value_mean"].mean().min(),
        df_hourly.groupby(["hour", "month"])["value_mean"].mean().max()
    )
    ...
    plot_all_regions_seasonal(df_hourly, signal, figures_dir)
