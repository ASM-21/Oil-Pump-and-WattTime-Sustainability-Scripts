...
Output (in run folder):
    ...
    - figures/06_seasonal_savings_distribution.png
    - figures/06_optimal_hour_by_season.png
    - figures/06_consistency_heatmap.png
    - figures/06_cumulative_savings.png
"""
...
def plot_seasonal_savings_distribution(results: dict, signal: str, figures_dir: Path):
    """Box plot showing distribution of daily scheduling savings by region and season.
    Reveals both magnitude and reliability of savings potential."""
    
    if "scheduling" not in results:
        print("  No scheduling data for savings distribution plot")
        return
    
    unit_label = get_unit_label(signal)
    scheduling = results["scheduling"]
    data = scheduling[(scheduling["job_duration_hours"] == 2.0) & 
                      (scheduling["shift"] == "day")].copy()
    
    if len(data) == 0:
        print("  No day-shift 2hr data for savings distribution plot")
        return
    ...
    for ax, region in zip(axes, regions):
        ...
        data_by_season = [region_data[region_data["season"] == s]["savings_vs_baseline_pct"].dropna().values
                          for s in seasons]
    ...
    axes[0].set_ylabel("Scheduling Savings (%)", fontsize=12)
    fig.suptitle("Distribution of Daily Scheduling Savings by Season\n(Day Shift, 2hr Job)", fontsize=13)
    plt.tight_layout()
    
    outpath = figures_dir / "06_seasonal_savings_distribution.png"
    plt.savefig(outpath, dpi=FIGURE_DPI)
    plt.close()
    print("  Saved: 06_seasonal_savings_distribution.png")


def plot_optimal_hour_by_season(results: dict, signal: str, figures_dir: Path):
    """Dot plot showing how the optimal scheduling hour shifts by season for each region.
    Reveals whether seasonal heuristic updates are needed."""
    
    if "scheduling" not in results:
        print("  No scheduling data for optimal hour plot")
        return
    ...
    fig, ax = plt.subplots(figsize=(10, max(5, len(regions) * 1.2)))
    ...
    for i, region in enumerate(regions):
        ...
        for j, season in enumerate(seasons):
            ...
            # Modal optimal hour
            modal_hour = season_data["optimal_start_hour"].mode()
            if len(modal_hour) == 0:
                continue
            modal_hour = int(modal_hour.iloc[0])
            
            # Spread: what fraction land within ±1 hour
            within_1hr = ((season_data["optimal_start_hour"] >= modal_hour - 1) & 
                          (season_data["optimal_start_hour"] <= modal_hour + 1)).mean()
            
            y = base_y + (j - 1.5) * 0.25
            ax.scatter(modal_hour, y, color=SEASON_COLORS[season], 
                       s=150, zorder=5, edgecolors='black', linewidth=0.5)
            
            # Error bar showing ±1hr consistency as alpha
            ax.plot([modal_hour - 1, modal_hour + 1], [y, y],
                    color=SEASON_COLORS[season], linewidth=3, alpha=within_1hr * 0.6 + 0.1)
        
        y_positions.append(base_y)
        y_labels.append(REGIONS.get(region, {}).get("name", region))
    
    ax.set_yticks(y_positions)
    ax.set_yticklabels(y_labels, fontsize=11)
    ax.set_xlabel("Optimal Start Hour (24hr)", fontsize=12)
    ...
    for season in seasons:
        ax.scatter([], [], color=SEASON_COLORS[season], s=80, label=season.capitalize(),
                   edgecolors='black', linewidth=0.5)
    ax.legend(loc="upper right", fontsize=9)
    
    ax.set_title("Optimal Scheduling Hour by Season\n(Day Shift, 2hr Job — bars show ±1hr window)", fontsize=13)
    plt.tight_layout()
    
    outpath = figures_dir / "06_optimal_hour_by_season.png"
    plt.savefig(outpath, dpi=FIGURE_DPI)
    plt.close()
    print("  Saved: 06_optimal_hour_by_season.png")


def plot_consistency_heatmap(heuristics: pd.DataFrame, signal: str, figures_dir: Path):
    """Heatmap of heuristic consistency (% days the recommended hour is optimal)
    by region and season. High = safe to use static rule. Low = need forecasts."""
    
    if heuristics is None or len(heuristics) == 0:
        print("  No heuristics data for consistency heatmap")
        return
    ...
    # Pivot: rows=region, cols=season
    seasons_order = ["winter", "spring", "summer", "fall"]
    regions_in_data = sorted(day_shift["region"].unique())
    
    pivot = day_shift.pivot_table(
        index="region", columns="season", values="consistency_pct", aggfunc="first"
    )
    # Reorder
    pivot = pivot.reindex(index=regions_in_data, 
                          columns=[s for s in seasons_order if s in pivot.columns])
    
    fig, ax = plt.subplots(figsize=(8, max(4, len(regions_in_data) * 0.8 + 2)))
    
    im = ax.imshow(pivot.values, aspect='auto', cmap='RdYlGn', vmin=0, vmax=100)
    
    # Labels
    ax.set_xticks(range(len(pivot.columns)))
    ax.set_xticklabels([s.capitalize() for s in pivot.columns], fontsize=11)
    ax.set_yticks(range(len(pivot.index)))
    ax.set_yticklabels([REGIONS.get(r, {}).get("name", r) for r in pivot.index], fontsize=11)
    ...
    ax.set_title("Heuristic Consistency by Region and Season\n(% days recommended hour is actually optimal — Day Shift, 2hr Job)", 
                 fontsize=12)
    plt.tight_layout()
    
    outpath = figures_dir / "06_consistency_heatmap.png"
    plt.savefig(outpath, dpi=FIGURE_DPI)
    plt.close()
    print("  Saved: 06_consistency_heatmap.png")


def plot_cumulative_savings(results: dict, signal: str, figures_dir: Path):
    """Cumulative CO2 savings over time by region from following the heuristic.
    Makes annual impact tangible."""
    
    if "scheduling" not in results:
        print("  No scheduling data for cumulative savings plot")
        return
    ...
    # Need a date column to plot over time
    date_col = None
    for col in ["date", "day", "point_time"]:
        if col in data.columns:
            date_col = col
            break
    
    if date_col is None:
        print("  No date column found in scheduling data — skipping cumulative savings plot")
        return
    
    data[date_col] = pd.to_datetime(data[date_col])
    data = data.sort_values(date_col)
    
    # Determine savings column — prefer CO2 grams, fall back to %
    if signal == "co2_moer" and "saved_co2_g" in data.columns:
        value_col = "saved_co2_g"
        ylabel = "Cumulative CO₂ Saved (kg)"
        scale = 1 / 1000  # g -> kg
        title_suffix = f"(Ref: {REFERENCE_JOB_KWH} kWh job)"
    else:
        value_col = "savings_vs_baseline_pct"
        ylabel = "Cumulative Savings (sum of daily %)"
        scale = 1
        title_suffix = ""
    ...
    for region in regions:
        rdata = data[data["region"] == region].sort_values(date_col)
        cumulative = (rdata[value_col] * scale).cumsum()
        ax.plot(rdata[date_col].values, cumulative.values,
                color=REGION_COLORS.get(region, "#333333"),
                linewidth=2, label=REGIONS.get(region, {}).get("name", region))
    
    ax.set_xlabel("Date", fontsize=12)
    ax.set_ylabel(ylabel, fontsize=12)
    ax.set_title(f"Cumulative Scheduling Savings Over Time\n(Day Shift, 2hr Job) {title_suffix}", fontsize=13)
    ...
    outpath = figures_dir / "06_cumulative_savings.png"
    plt.savefig(outpath, dpi=FIGURE_DPI)
    plt.close()
    print("  Saved: 06_cumulative_savings.png")
...
def main():
    ...
    plot_seasonal_savings_distribution(results, signal, figures_dir)
    plot_optimal_hour_by_season(results, signal, figures_dir)
    plot_consistency_heatmap(heuristics, signal, figures_dir)
    plot_cumulative_savings(results, signal, figures_dir)
